...
export default function StaffJabatanDashboard() {
  ...
  // 2️⃣ The raw list of “application” objects from our API
  const [applications, setApplications] = useState<any[]>([]);

  // 3️⃣ Which application ID is currently selected?
  const [selectedAppId, setSelectedAppId] = useState<string | null>(null);

  // 4️⃣ Filter and sort dropdowns
  const [statusFilter, setStatusFilter] = useState("All"); // "All" | "pending" | "accepted" | "rejected"
  const [orderFilter, setOrderFilter] = useState("desc");   // "asc" | "desc"

  // 5️⃣ Our “staffId” (hard‐code for now; replace with real auth later)
  const staffId = "YOUR_STAFF_JABATAN_UUID";

  // 6️⃣ Fetch the applications list from our API
  async function fetchApplications() {
    try {
      let url = "/api/staffjabatan/get-applications";
        ...
        <div className="flex flex-col items-center gap-3 h-full bg-off-white border border-gray-200 p-2 rounded-lg overflow-y-auto">
          {applications.length === 0 ? (
            ...
            applications.map((app) => (
              ...
              >
                <h6 className="font-medium w-44 overflow-hidden text-ellipsis whitespace-nowrap">
                  {app.id.slice(0, 8) + "..."}
                </h6>
                <div className="flex flex-row items-center gap-1.5 text-gray-500 text-sm">
                  ...
                  {app.phase_status === "2" && (
                    <strong className="text-[#FDD660]">Phase 02</strong>
                  )}
                  {app.phase_status === "3" && (
                    <strong className="text-[#AF52DE]">Phase 03</strong>
                  )}
                  {app.phase_status === "4" && (
                    <strong className="text-[#FF2D55]">Phase 04</strong>
                  )}

                  {/* If status === accepted but phase < 4, show “Accepted” */}
                  {app.status === "accepted" && app.phase_status !== "4" && (
                    <strong className="text-[#34C759]">Accepted</strong>
                  )}
                  {app.status === "rejected" && (
                    <strong className="text-[#B3261E]">Rejected</strong>
                  )}
