...
export default function ModalP4({ applicationId, staffId, onClose }: ModalP4Props) {
	...
	async function handleConfirmPhase4() {
		...
		try {
			...
			const markRes = await fetch("/api/staff-jabatan/enter-mark", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
			...
			const inviteRes = await fetch("/api/staff-jabatan/approve-application", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
		...
		<div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50">
			<div className="bg-white w-full h-162 max-w-6xl rounded-xl p-2 relative space-y-2">
									...
									<div key={phase} className="flex items-center gap-2 cursor-pointer group">
										<span
											className={`w-4 h-4 rounded-full border-2 transition group-hover:scale-110 ${
												isCurrent ? "bg-[#FF2D55] border-[#FF2D55]" : "border-gray-400"
											}`}
										/>
										<span className={`font-semibold ${isCurrent ? "text-[#FF2D55]" : ""}`}>{phase}</span>
									</div>
								);
							})}
						</div>
				...
				<div className="flex flex-row justify-end-safe gap-4">
					<Button text="Confirm Date" color="dark-blue" onClick={handleConfirmPhase4} disabled={isSubmitting} />
					<Button text="Reject" color="danger" onClick={handleRejectPhase4} disabled={isSubmitting} />
				</div>
			</div>
		</div>
	);
